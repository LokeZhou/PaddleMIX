...
class VCtrlPatchEmbed(paddle.nn.Layer):
    ...
    def __init__(self, patch_size: int=2, in_channels: int=16, embed_dim:
        int=1920, bias: bool=True) ->None:
        super().__init__()
    ...
    def forward(self, image_embeds: paddle.Tensor):
        ...
        image_embeds = image_embeds.reshape([batch, num_frames, *tuple(
...
class VCtrlLayerNormZero(paddle.nn.Layer):
    ...
    def __init__(self, conditioning_dim: int, embedding_dim: int,
        ...
        self.linear = paddle.nn.Linear(in_features=conditioning_dim,
            out_features=3 * embedding_dim, bias_attr=bias)
        self.norm = paddle.nn.LayerNorm(normalized_shape=embedding_dim,
            epsilon=eps, weight_attr=elementwise_affine, bias_attr=
            elementwise_affine)

    def forward(self, hidden_states: paddle.Tensor, temb: paddle.Tensor
        ) ->paddle.Tensor:
        shift, scale, gate = self.linear(self.silu(temb)).chunk(chunks=3,
            axis=1)
...
class VCtrlAttnProcessor2_0:
    ...
    def __init__(self):
        if not hasattr(paddle.nn.functional, 'scaled_dot_product_attention'):
            raise ImportError(
                'VCtrlAttnProcessor2_0 requires Paddle >2.5.'
                )

    def __call__(self, attn: Attention,
        hidden_states: paddle.Tensor, attention_mask: Optional[paddle.
        Tensor]=None, image_rotary_emb: Optional[paddle.Tensor]=None,
        encoder_hidden_states: Optional[paddle.Tensor]=None) ->paddle.Tensor:
...
class VCtrlBlock(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, num_attention_heads: int,
        attention_head_dim: int, time_embed_dim: int, dropout: float=0.0,
        activation_fn: str='gelu-approximate', attention_bias: bool=False,
        qk_norm: bool=True, norm_elementwise_affine: bool=True, norm_eps:
        float=1e-05, final_dropout: bool=True, ff_inner_dim: Optional[int]=
        None, ff_bias: bool=True, attention_out_bias: bool=True):
        super().__init__()
        self.norm1 = VCtrlLayerNormZero(time_embed_dim, dim,
            norm_elementwise_affine, norm_eps, bias=True)
        
        self.attn1 = Attention(query_dim
            =dim, dim_head=attention_head_dim, heads=num_attention_heads,
            qk_norm='layer_norm' if qk_norm else None, eps=1e-06, bias=
            attention_bias, out_bias=attention_out_bias, processor=
            VCtrlAttnProcessor2_0())
        self.norm2 = VCtrlLayerNormZero(time_embed_dim, dim,
            norm_elementwise_affine, norm_eps, bias=True)
        self.ff = FeedForward(dim, dropout=
            dropout, activation_fn=activation_fn, final_dropout=
            final_dropout, inner_dim=ff_inner_dim, bias=ff_bias)
...
class VCtrlModel(ModelMixin,ConfigMixin):
    ...
    def __init__(self, num_attention_heads: int=30, attention_head_dim: int
        =64, flip_sin_to_cos: bool=True, freq_shift: int=0, time_embed_dim:
        int=512, timestep_activation_fn: str='silu', patch_size: int=2,
        in_channels: int=16, extra_conditioning_channels: int=1, num_layers:
        int=6, dropout: float=0.0, activation_fn: str='gelu-approximate',
        attention_bias: bool=True, norm_elementwise_affine: bool=True,
        norm_eps: float=1e-05, use_rotary_positional_embeddings: bool=True,
        ...
        self.sample_patch_embed = VCtrlPatchEmbed(patch_size, in_channels *
            2 if image_to_video else in_channels, inner_dim, bias=True)
        self.cond_patch_embed = VCtrlPatchEmbed(patch_size, in_channels +
            extra_conditioning_channels, inner_dim, bias=True)
        self.transformer_blocks = paddle.nn.LayerList(sublayers=[VCtrlBlock
            (dim=inner_dim, num_attention_heads=num_attention_heads,
            attention_head_dim=attention_head_dim, time_embed_dim=
            time_embed_dim, dropout=dropout, activation_fn=activation_fn,
            attention_bias=attention_bias, norm_elementwise_affine=
            norm_elementwise_affine, norm_eps=norm_eps) for _ in range(
            num_layers)])
    ...
    def enable_forward_chunking(self, chunk_size: Optional[int]=None, dim:
        int=0) ->None:
        """
        ...
        def fn_recursive_feed_forward(module: paddle.nn.Layer, chunk_size:
            int, dim: int):
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = paddle.distributed.fleet.utils.recompute(create_custom_forward(block), hidden_states, t_emb,image_rotary_emb, **ckpt_kwargs)
            else:
