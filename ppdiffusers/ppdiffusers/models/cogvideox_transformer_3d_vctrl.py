...
from ppdiffusers.models.attention_processor import AttentionProcessor, CogVideoXAttnProcessor2_0, FusedCogVideoXAttnProcessor2_0
from ppdiffusers.models.embeddings import CogVideoXPatchEmbed, TimestepEmbedding, Timesteps
...
class CogVideoXBlock(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            heads=num_attention_heads, qk_norm='layer_norm' if qk_norm else None, 
            eps=1e-06, 
            bias=attention_bias, 
...
class CogVideoXTransformer3DVCtrlModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self, num_attention_heads: int=30, attention_head_dim: int
        ...
        self.patch_embed = CogVideoXPatchEmbed(
            patch_size=patch_size, in_channels=in_channels, embed_dim=
            inner_dim, text_embed_dim=text_embed_dim, bias=True,
            ...
            max_text_seq_length, spatial_interpolation_scale=
            spatial_interpolation_scale, temporal_interpolation_scale=
            temporal_interpolation_scale, use_positional_embeddings=not
        ...
        self.norm_out = AdaLayerNorm(
            embedding_dim=time_embed_dim, output_dim=2 * inner_dim,
            norm_elementwise_affine=norm_elementwise_affine, norm_eps=
            norm_eps, chunk_dim=1)
    ...
    def forward(
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if block_vctrl_residuals is not None:
                if vctrl_layout_type == "even":
                    interval_control = len(self.transformer_blocks) // len(block_vctrl_residuals)
                    hidden_states = hidden_states + paddle.nn.functional.adaptive_avg_pool1d(block_vctrl_residuals[i // interval_control], hidden_states.shape[-1])
                elif vctrl_layout_type == "spacing":
                    interval_control = len(self.transformer_blocks) // len(block_vctrl_residuals)
                    if i % interval_control == 0:
                        hidden_states = hidden_states + paddle.nn.functional.adaptive_avg_pool1d(
                            block_vctrl_residuals[i // interval_control],
                            hidden_states.shape[-1]
                        )
                elif vctrl_layout_type == "end":
                    start_idx = len(self.transformer_blocks) - len(block_vctrl_residuals)
                    if i >= start_idx:
                        residual_idx = i - start_idx
                        hidden_states = hidden_states + paddle.nn.functional.adaptive_avg_pool1d(
                            block_vctrl_residuals[residual_idx], hidden_states.shape[-1]
                        )
                else:
        ...
        if not self.config.use_rotary_positional_embeddings:
            ...
            hidden_states = paddle.concat(x=[encoder_hidden_states,hidden_states], axis=1)
            hidden_states = self.norm_final(hidden_states)
            hidden_states = hidden_states[:, text_seq_length:]
        ...
        output = output.transpose(perm=[0, 1, 4, 2, 5, 3, 6]).flatten(start_axis=5, stop_axis=6).flatten(start_axis=3, stop_axis=4)
