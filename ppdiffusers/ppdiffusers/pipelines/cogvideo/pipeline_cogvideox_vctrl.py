...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> )
        >>> pipe = CogVideoXVCtrlPipeline.from_pretrained("THUDM/CogVideoX-2b", vctrl=vctrl, torch_dtype=torch.float16).to("cuda")
...
class CogVideoXVCtrlPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.vctrl_image_processor = (
            VaeImageProcessor(vae_scale_factor=self.
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_frames,
        height, width, dtype, generator, latents=None):
        shape = (batch_size, (num_frames - 1) // self.
            vae_scale_factor_temporal + 1, num_channels_latents, height //
            self.vae_scale_factor_spatial, width // self.
    ...
    def check_inputs(self, prompt, height, width, negative_prompt,
        callback_on_step_end_tensor_inputs, prompt_embeds=None,
    ...
    def _prepare_rotary_positional_embeddings(self, height: int, width: int,
        num_frames: int) ->Tuple[paddle.Tensor, paddle.Tensor]:
        ...
        freqs_cos, freqs_sin = (get_3d_rotary_pos_embed(embed_dim=self.transformer.config.
    ...
    def _prepare_v_ctrl_rotary_positional_embeddings(self, height: int,
        ...
        grid_crops_coords = get_resize_crop_region_for_grid((grid_height,
            grid_width), base_size_width, base_size_height)
        freqs_cos, freqs_sin = (get_3d_rotary_pos_embed(embed_dim=self.vctrl.config.
            attention_head_dim, crops_coords=grid_crops_coords, grid_size=(
            grid_height, grid_width), temporal_size=num_frames))
    ...
    def __call__(
        ...
        latents = self.prepare_latents(batch_size * num_videos_per_prompt,latent_channels, num_frames, height, width, prompt_embeds.dtype, generator, latents)
        ...
        v_cond_rotary_emb = self._prepare_v_ctrl_rotary_positional_embeddings(
            height, width, v_cond.shape[1]
            ) if self.vctrl.config.use_rotary_positional_embeddings else None
