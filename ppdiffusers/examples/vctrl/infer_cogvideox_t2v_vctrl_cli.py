...
if __name__ == '__main__':
    args = parse_args()
    assert (args.control_images_folder is None) ^ (args.control_video_path is None), 'must and only one of [control_images_folder, control_video_path] should be given'
    ...
    if args.vctrl_path.endswith('.pdparams'):
        ...
        vctrl = VCtrlModel.from_pretrained(args.pretrained_model_name_or_path, subfolder='vctrl',low_cpu_mem_usage=True)
    ...
    if args.transformer_path:
        ...
        pipeline = CogVideoXVCtrlPipeline.from_pretrained(args.pretrained_model_name_or_path, vctrl=vctrl,paddle_dtype=paddle.float16)
