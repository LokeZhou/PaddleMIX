...
def save_vid_side_by_side(batch_output, validation_control_images,
    output_folder, fps):
    flattened_batch_output = [img for sublist in batch_output for img in
        sublist]
    ...
    def get_concat_h(im1, im2):
        dst = Image.new('RGB', (im1.width + im2.width, max(im1.height, im2.
            height)))
        dst.paste(im1, (0, 0))
...
def parse_args():
    parser = argparse.ArgumentParser(description=
        'Generate a video from a text prompt using CogVideoX with VCTRL')
    parser.add_argument('--pretrained_model_name_or_path', type=str,
        default=None, required=True)
    parser.add_argument('--control_images_folder', type=str,
        default=None, required=False)
    parser.add_argument('--control_video_path', type=str,
        default=None, required=False)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "the validation mask images from video"
        ),
    )
    ...
    parser.add_argument('--vctrl_config', type=str, default=None, help=
        'the config file for vctrl')
...
def load_tensor(tensor_path):
    if os.path.splitext(tensor_path)[1] == '.bin' or os.path.splitext(
        tensor_path)[1] == '.pt':
...
if __name__ == '__main__':
    args = parse_args()
    assert (args.control_images_folder is None) ^ (args.control_video_path is None
    ...
    if args.vctrl_path.endswith('.pdparams'):
        ...
        vctrl = VCtrlModel.from_pretrained(args.pretrained_model_name_or_path, subfolder='vctrl',low_cpu_mem_usage=True, torch_dtype='float16')
    ...
    video = pipeline(
        ...
        vctrl_layout_type=args.vctrl_layout_type).frames[0]
    final_result.append(video)
    save_vid_side_by_side(final_result, validation_control_images[:
        num_frames], args.output_dir, fps=args.fps)
