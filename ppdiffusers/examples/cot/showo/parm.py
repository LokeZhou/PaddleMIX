...
class ShowoParm(Showo):
    ...
    def t2i_generate(
        ...
        **kwargs,
        ...
        for _ in tqdm(range(config.eval_num)):
            no_prob_list = [1.0] * config.search_num
            potential_list = [False] * config.search_num
            image_paths = []
            for iteration_count in range(config.search_num):
                ...
                for step in range(timesteps):
                    ...
                    if clear:
                        ...
                        filtered, score, clear = selector.parm(prompts_batch, image_path, clear=clear)
                    sampled_ids_copy = sampled_ids.clone()
                    sampled_ids_copy = paddle.clip(
                        x=sampled_ids_copy,
                        max=config.model.showo.codebook_size - 1,
                    ...
                    if step == timesteps - 1:
                        no_prob_list[iteration_count] = score
                        potential_list[iteration_count] = filtered
                        sampled_ids_copy = sampled_ids.clone()
                        sampled_ids_copy = paddle.clip(
                            x=sampled_ids_copy,
                            max=config.model.showo.codebook_size - 1,
            ...
            if true_indices:
                ...
                for i, image_path in enumerate(potential_paths):
                    ...
                    if score > highest_score_yes:
                        ...
                        best_index = i
                os.system("cp " + potential_paths[best_index] + " " + os.path.join(sample_path, f"{_:04d}.png"))
            else:
                best_index = min(range(len(no_prob_list)), key=lambda i: no_prob_list[i])
                os.system("cp " + image_paths[best_index] + " " + os.path.join(sample_path, f"{_:04d}.png"))
...
def main(opt):
    ...
    print(f"Starting rank={rank}, world_size={paddle.distributed.get_world_size()}.")
    if opt.model == "show-o":
        ...
        model = ShowoParm.from_pretrained(opt.dpo_model_path, dtype=opt.dtype)
        model.eval()
